...
.navbar-default .navbar-brand, .navbar-default .navbar-brand:hover {
	...
	<div class="content">
		<div class="header">
			...
			<h1 class="page-title">查看修改-思维导图</h1>
			<ul class="breadcrumb">
				<li><a href="loginshow.do">Home</a></li>
				<li><a href="thinkAll.do">思维导图</a></li>
				<li class="active">查看修改</li>
			</ul>

		</div>

		<div id="sample">
			<input id="thinking_id" type="hidden" value="${thinkinginfo.thinkId}" />
			<input id="thinking_username" type="hidden"
				value="${thinkinginfo.thinkUserName}" /> 
			<input id="thinking_flag" type="hidden" value="${thinkinginfo.thinkFlag}" />
			<input value="${thinkinginfo.thinkTheme}" id="thinking_title" type="text"
			...
			<div>
				...
				<c:if test="${loginUser.upd==1}">
					<button style="float: right;" onclick="updateSubmit()">修改提交</button>
				</c:if>
		...
		function updateSubmit() {
			...
			if (localThinkData != submitThinkData) {
				if ($('#thinking_title').val() != null
					...
					$.ajax({
						type : "post",
						url : "thinking_update.do",
						data : {
							thinkId : $('#thinking_id').val(),
							thinkTheme : $('#thinking_title').val(),
							thinkFlag : $('#thinking_flag').val(),
							thinkdate : time,
						...
						success : function(data) {
							...
							if (data.status == "Success") {
								success_prompt("Success：思维导图修改成功！", 1200);
								$('#thinking_flag').val(data.thinking_flag);
							} else {
								fail_prompt("Error:思维导图修改失败！", 1200);
							}
						},
						error : function(data) {
							fail_prompt("Error:思维导图修改失败！", 1200);
				...
				if (submitThinkData == 0)
					...
					warning_prompt("Warning:您已修改提交，请勿重复提交！", 1200);
			}
		}
	</script>
	<div class="alert"></div>
